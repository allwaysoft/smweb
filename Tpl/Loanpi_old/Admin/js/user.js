...
$(document).ready(function(){ 
	  ...
	  $('#listTable').dataTable().fnDestroy(); 
			 $('#listTable').dataTable( { 
					  ...
					  "fixedHeader": true,
					   "responsive": true,
					 "ajax":{
						  "url": "/loanpiadmin.php/User/user_list",
					  },
					 "order": [[ 0, "desc" ]],
					 "columns": [
							{"data": "create_time"},
					...
					"fnRowCallback":function(nRow,aData,iDataIndex){
						$('td:eq(0)',nRow).addClass("tableDate");
						 	var html = ""; 
							 html += "<button class='btn btn-info btn-xs'  onclick=_funEdit('"+aData.id+"') type='button' name='_edit'>编辑</button>";
							 html += " <button class='btn btn-warning btn-xs'  onclick=_funRepass('"+aData.id+"') type='button' name='_edit'>修改密码</button>"; 
	...
	$("button[name=addBut]").click(function() { 
		BootstrapDialog.show({
            ...
            message: $('<div></div>').load("/loanpiadmin.php/User/user_add") 
        }); 
	});
	////add end
...
function _funEdit(val) { 
		BootstrapDialog.show({
            ...
            message: $('<div></div>').load("/loanpiadmin.php/User/user_edit/id/"+val) 
        }); 
	};
	////edit end
	// edit
function _funRepass(val) { 
		BootstrapDialog.show({
            ...
            message: $('<div></div>').load("/loanpiadmin.php/User/user_pass/id/"+val) 
        }); 
	};
	////add end
	// function del
    function _funDel(dialogItself) { 
            ...
            if(result) {
				dialogItself.close();
				var id = $('#id').val();
		 		 post_url = "/loanpiadmin.php/User/user_del_do"; 
