...
class ServiceController extends Controller {
    ...
    public function upstatusdo() {
        ...
        $server = new \SoapServer('./UpStatusFromPi.wsdl', array('soap_version' => SOAP_1_2)); ##此处的Service.wsdl文件是上面生成的, array('soap_version' => SOAP_1_2) 
        $server->setClass('FromPI'); //注册Service类的所有方法   
        $server->handle(); //处理请求  
        $logs['user'] = 'FromPI';
        $logs['title'] = 'webservice 调用';
        $logs['status'] = '调用成功';
        $logs['operating'] = '';
        ...
        if ($data['AGENT_CODE'] && $data['LOAN_NUM']) {
            ...
            if ($list) {
                ...
                if ($data['LOAN_STATUS'] == 5) {
                    ...
                    $udata['reg_amount'] = $data['LOAN_AMOUNT'];
                }
                //   5 贷款审批未通过
...
/**
     ...
     * 功能：PI webservice testing 
      +----------------------------------------------------------
     */
	public function UpStatusDotest() {
        ...
        curl_setopt($ch, CURLOPT_URL, "http://10.192.1.10/loanpi.php/Service/UpStatusToLocal");
    ...
    function soaptest() {
        ...
        //testing  soap
        /////////////////////
        $soapURL = "http://w.semir.cn/UpStatusFromPi.wsdl"; //"http://localhost:8082/MI_OA2SAP_Psc_Send.wsdl"; //
        ...
        //  echo $soap->debug_str;
        $params = array('AGENT_CODE' => '11CP1', 'LOAN_NUM' => '0000000065', 'LOAN_TYPE' => 'S', 'LOAN_TYPE' => 'S', 'LOAN_STATUS' => '5', 'LOAN_REASON' => 'Ssss', 'LOAN_DATE_FROM' => '20160505', 'LOAN_DATE_TO' => '20160909', 'LOAN_CYCLE' => '30', 'LOAN_AMOUNT' => '10000', 'YL1' => '', 'YL2' => '', 'YL3' => '', 'YL4' => '', 'YL5' => '');
