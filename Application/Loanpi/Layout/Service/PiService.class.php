...
class PiService extends Model {
    ...
    protected function _initialize() {
        parent::_initialize();
        $this->soapURL = "./MI_SW2SAP_Crd_Send.wsdl"; //"MI_SW2SAP_Crd_Send.wsdl"; //
    ...
    public function get_loanstatus($data) {
        ...
        $reData = $this->soap_pi($soapFunctionParameters);
          //换算为万元
        $reData->LOAN_AMOUNT = round($reData->LOAN_AMOUNT / 10000, 2); 
    ...
    public function get_limit($data) {
        ...
        $reData = $this->soap_pi($soapFunctionParameters);
          //换算为万元
        $reData->LOAN_LIMIT = round($reData->LOAN_LIMIT / 10000, 2); 
        return $reData;
    }
    ...
    public function to_register($data) {
         ...
         // 金额换算
        $reg_amount = $data['reg_amount'] * 10000;
		
        $soapFunctionParameters = Array('AGENT_CODE' => $data['dl_code'], 'LOAN_DATE_FROM' => date('Ymd', strtotime($data['reg_start_time'])), 'LOAN_CYCLE' => $data['reg_cycle'], 'LOAN_AMOUNT' => $reg_amount, 'LOAN_TYPE' => $data['type'], 'type' => '2');
        $reData = $this->soap_pi($soapFunctionParameters);
        return $reData;
    }
    ...
    public function to_repayment($data) {
        // 金额换算
        $rep_amount = $data['rep_amount'] * 10000;
        $soapFunctionParameters = Array('AGENT_CODE' =>  $data['dl_code'], 'LOAN_NUM' => $data['sap_code'],  'REPAY_AMOUNT' => $rep_amount, 'LOAN_TYPE' => $data['rep_type'], 'type' => '4'); // 还款申请
        $reData = $this->soap_pi($soapFunctionParameters);
        return $reData;
    }
