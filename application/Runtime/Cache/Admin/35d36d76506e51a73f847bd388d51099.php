    ...
    <table width="100%" cellpadding="0" cellspacing="0" class="list">
      <thead>
        <tr>
          ...
          <th width="50">序号</th>
          <th width="250">图片名称</th>
          <th width="250">图片预览</th>
      ...
      <?php if(is_array($dirnames)): $i = 0; $__LIST__ = $dirnames;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr >
         <td><input type="checkbox" name="name[]" value="<?php echo ($vo); ?>" /></td>
          <td><?php echo ($i); ?></td>
          <td><?php echo ($vo); ?></td>
          <td><a class="preview" href="/Upload/<?php echo ($name); ?>/<?php echo ($vo); ?>" title="<?php echo (substr($vo,0,-4)); ?>"><img src="/Upload/<?php echo ($name); ?>/<?php echo ($vo); ?>" width="16" height="20" /></a></td>      
          <td width="200" style="text-align:center;">
          <a href="<?php echo U('Photo/picdel',array('dir'=>$name,'name'=>$vo));?>" onclick="return confirm('确定删除该图片吗？')" class="edit">删除</a>
