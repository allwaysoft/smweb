...
<div id="content" class="white">
 <div class="mbxie"><a href="" class="first_a" ><img src="/Public/admin/images/back.jpg" width="25" height="12" />返回上一页</a><img src="/Public/admin/images/home1.jpg" width="20" height="12" /><a href="<?php echo U('Admin/Main/main');?>"  style=" padding-left:3px;">首页</a>><a href="javascript:void(0)">系统设置</a>><a href="index">超链接设置</a></div>
  <div class="bloc">
    <div class="content">
        <dl class="dl_line">
			...
			<dd >
			  ...
			  <td>
				<select id="type_id" name="type_id" class="valid">
				  ...
				  <option <?php if($type_id == 1): ?>selected="selected"<?php endif; ?> value=1>﹄ 顶部</option>
				  <option <?php if($type_id == 2): ?>selected="selected"<?php endif; ?> value=2>﹄ 底部</option>
				  <option <?php if($type_id == 3): ?>selected="selected"<?php endif; ?> value=3>﹄ 右侧</option>
				  <option <?php if($type_id == 4): ?>selected="selected"<?php endif; ?> value=4>﹄ 实用工具</option>
				</select>
			  </td>
			</dd>
        </dl>
	    <form method="post" action="<?php echo U('Links/del');?>" id="form1">
		<table id="alist" border="0" cellpadding="0" cellspacing="0" width="100%" class="list" style="float:left" >  
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php if($vo["type_id"] == 1): ?>顶部<?php elseif($vo["type_id"] == 2): ?>底部<?php elseif($vo["type_id"] == 3): ?>右侧<?php else: ?>实用工具<?php endif; ?></td>
				<td><?php if($vo["ispic"] == 0): ?>文字<?php else: ?>图片<?php endif; ?></td>
				<td><?php if($vo["isarea"] == 0): ?>否<?php else: ?>是<?php endif; ?></td>
				<td><?php if($vo["isin"] == 0): ?>站外<?php else: ?>站内<?php endif; ?></td>
				<td><?php if($vo["url"] == ''): ?>无<?php else: echo ($vo["url"]); endif; ?></td>
				<td><?php if($vo["display"] == 0): ?>否<?php else: ?>是<?php endif; ?></td>
				<td class="actions" style="text-align:center; margin-left:10px;"><a href = "<?php echo U('Admin/Links/edit',array('id'=>$vo['id']));?>" >修改</a> <!-- | <a href="<?php echo U('Admin/Download/viddel',array('id'=>$vo['id']));?>" title="删除">删除</a> --></td>
...
$('#type_id').change(function(){
	...
	var url="<?php echo U('Links/index');?>";
