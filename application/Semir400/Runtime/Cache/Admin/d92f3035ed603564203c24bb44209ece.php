   ...
   <ul class="nav nav-pills nav-stacked" role="tablist"> 
    ...
    <div class="col-xs-10">
     <div class="topRightBut pull-right">
        <button type="button" name="addBut" class="btn btn-success  btn-sm"><i class="fa fa-cube" aria-hidden="true"></i> 添加主题</button>
      </div>
     <h1><i class="fa fa-cube" aria-hidden="true"></i> 投诉主题管理</h1>
     <div class="h20"></div>
      <form action="" method=" ">
 ...
 $('#listTable').dataTable().fnDestroy(); 
  $('#listTable').dataTable( { 
					  ...
					  "fixedHeader": true,
					   "responsive": true,
					 "ajax":{
						  "url": "/semir400admin.php/System/subject_list",
					  },
					 "order": [[ 0, "desc" ]],
					 "columns": [
							{"data": "id"},
...
// add and edit gallery end 
	$("button[name=addBut]").click(function() { 
		BootstrapDialog.show({
            ...
            message: $('<div></div>').load("/semir400admin.php/System/subject_add") 
        }); 
	});
	////add end
...
function _funEdit(val) { 
		BootstrapDialog.show({
            ...
            message: $('<div></div>').load("/semir400admin.php/System/subject_edit/id/"+val) 
        }); 
	};
	////edit end
 // function del
    function _funDel(dialogItself) { 
            ...
            if(result) {
				dialogItself.close();
				var id = $('#id').val();
		 		 post_url = "/semir400admin.php/System/subject_del_do"; 
